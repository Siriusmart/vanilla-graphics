...
async function setupTemplate(packageName) {
	if(!fs.existsSync(`./meta/templates/${packageName}/package.json`)) return

	let manifestJSON = JSON.parse(fs.readFileSync(`./meta/templates/${packageName}/package.json`));

	if((manifestJSON.dependencies ?? []).length == 0 || fs.existsSync(`./meta/templates/${packageName}/node_modules`)) return

	console.log(`Installing npm dependencies for ${packageName}`)
            try {
...
async function setupTemplates() {
        for (let packageName of fs.readdirSync("./meta/templates")) {
			await setupTemplate(packageName);
		}
}
...
async function install(_projectPath, ...packagesToInstall) {
    ...
    if (fs.existsSync("./meta/templates"))
		...
		fs.mkdirSync("./meta/templates/", { recursive: true });
	...
	let alreadyInstalledSet = new Set(Object.keys(localManifests));

	for(let packageName of packagesToInstall) {
		if(toInstallSet.has(packageName) && !alreadyInstalledSet.has(packageName)) {
			siteJSON.templates ??= []
			siteJSON.templates.push(packageName);
		}
	}

	fs.writeFileSync("./site.json", JSON.stringify(siteJSON, null, 2))
...
module.exports = { install, setupTemplates };
